...
--AVG(rental_duration) OVER (PARTITION BY rental_duration) AS avg_duration
--FROM film;
...
--FROM (
 ...
 --JOIN film_category ON film.film_id = film_category.film_id
 --JOIN category ON film_category.category_id = category.category_id
 --GROUP BY category.category_id, film.title) AS rental_counts WHERE category_rank <= 3;
...
--SELECT customer_id, total_rentals, total_rentals - AVG(total_rentals) OVER () AS rental_difference FROM customer_rentals;
